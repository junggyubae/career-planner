...
- **Card convention (important):** a Darwinian card source is **not** stored in the consumer repo — `drwn card new` creates it under `~/.agents/drwn/sources/<scope>/<name>/` (manifest `card.json`), `drwn card publish` snapshots it to the local store, and `share-mind-card` pushes it to a git remote/catalog. **This repo is the *consumer*** (holds `memory/` + a `card.lock` pinning the applied card). The in-repo `card/` folder is a human-authoring **workspace**; the shippable card is generated from it via `drwn card new` + `add-skill --from card/skills/...`, then published/shared.
