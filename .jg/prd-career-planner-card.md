...
**Status:** Draft · **Card:** `@junggyubae/career-planner-card@0.2.0`
**Owner:** dev@greekwaters.io
**Last updated:** 2026-06-30
**Runtime:** Darwinian Mind Card running inside Claude Code (agent + file-based state)

> **Data model (v0.2.0):** `state · goal · action` (see §5). The card is the policy `π(action | state, goal)`. Some diagrams in §4/§6 predate the rename and still show `memory/…`/`output/…`; §5 is authoritative.
...
## 5. Data Design — `state · goal · action` (v0.2.0)

> **v0.2.0 model change.** The earlier `memory/` + `output/` layout is replaced by an RL-shaped model. The card is the **policy** `π(action | state, goal)`. Diagrams in §4/§6 that predate this use the old `memory/…`/`output/…` names; the data model of record is below.
...
flowchart TD
    ...
    S --> INT["interests.md"]
    S --> BEL["beliefs.md"]
    S --> UND["understanding.md<br/>(living self-synthesis)"]
...
| `state/beliefs.md` | values & how you work | Info Retrieval | **Finder (fit)** + **Alignment (SOP voice)** |
| `state/understanding.md` | living self-synthesis (read first) | Info Retrieval | Finder, Alignment |
...
- **FR-2:** The system must store all data as markdown files: `state/` (identity, experience, interests.md, beliefs.md, understanding.md, uploads/), `goal/` (goals.md), and `action/` (discovery/, applications/).
...
- **FR-4:** Info Retrieval must support an **interview path**: present a **list of focused questions** on a chosen topic (experience/interests/beliefs/goals/understanding), iterate with follow-ups, and refine/enrich the correct file.
